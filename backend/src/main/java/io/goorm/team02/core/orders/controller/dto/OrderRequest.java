...
public record OrderRequest(
		...
		@Schema(description = "배달 주소", example = "서울시 강남구 역삼동 123-45")
	...
	public Order toEntity() {
		...
		// user와 store는 서비스에서 ID로 조회해서 설정
		// order.setUser()와 order.setStore()는 서비스에서 처리
		...
		// 금액 정보는 서비스에서 계산해서 설정
		// order.setMenuTotalAmount(), order.setTotalAmount() 등은 서비스에서 처리
		
		// 주문 아이템들은 별도로 처리
		// order.setOrderItems()는 서비스에서 처리
	...
	public record OrderItemRequest(
			...
			@Schema(description = "주문 아이템 옵션 목록")
	...
	public record OrderItemOptionRequest(
			...
			@Schema(description = "추가 가격", example = "2000.00")
