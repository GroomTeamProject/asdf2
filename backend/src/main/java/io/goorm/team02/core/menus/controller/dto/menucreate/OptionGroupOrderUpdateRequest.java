...
@Setter
@NoArgsConstructor
@Schema(description = "옵션 그룹 순서 변경 요청")
public class OptionGroupOrderUpdateRequest {

    @NotNull(message = "이동할 옵션 그룹 ID는 필수입니다")
    @Schema(description = "이동할 옵션 그룹 ID", required = true, example = "1")
    private Long groupId;

    @NotNull(message = "새로운 위치는 필수입니다")
    @Min(value = 1, message = "새로운 위치는 1 이상이어야 합니다")
    @Schema(description = "새로운 위치 (1부터 시작)", required = true, example = "2")
